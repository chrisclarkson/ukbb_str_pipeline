...
data_old=data.frame(fread('data_500k.tsv.gz',header=T),stringsAsFactors=F)
data=data[((data$gene=='HTT') & (data$A2>=36)),]
data_old$VCF=basename(gsub('.vcf','',data_old$VCF))

icds=gsub(" ",'',gsub("\\]","",gsub("\\[","",gsub("'","",do.call('c',lapply(data$p41202_Diagnoses_._main_ICD10,function(x){return(unlist(strsplit(x,',')))}))))))
icds=icds[order(icds[!is.na(icds)])]
counts=table(icds)
icds=names(counts)[counts>=2]
# icds=unique(icds[order(icds[!is.na(icds)])])
