...
data_v2=read.table(args[1],sep='\t',
                   stringsAsFactors = F,header=T)

data_v0=read.table(args[2],sep='\t',
                   stringsAsFactors = F,header=T)
print(head(data_v2))
print(head(data_v0))
#RCv2 data:
eha1_rca1=data_v2[data_v2$EH_A1 == data_v2$RC_A1,]
# print(head(eha1_rca1))
...
#a1
# print(data_v2_sorted)
data_v2_sorted$GT1_imputed=NA
data_v2_sorted$GT2_imputed=NA
...
print((sum(data_v2_sorted$GT1_imputed=='imputed')+sum(data_v2_sorted$GT2_imputed=='imputed'))/(nrow(data_v2_sorted)*2))
print((sum(data_v2_sorted$GT1_imputed=='not imputed')+sum(data_v2_sorted$GT2_imputed=='not imputed'))/(nrow(data_v2_sorted)*2))

# print(head(data_v2_sorted))
data_v2_sorted$GT1_category=NA
...
data_v2_sorted$GT1_category[data_v2_sorted$GT1=="(CAG|CCG|CCT)"]<-"Q2_P1_loss"
...
out$bam_file=gsub("EH_","",out$bam_file)
write.table(unique(out), file = args[3],
